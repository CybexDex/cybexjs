...
let emitter = (0, _EmitterInstance.default)();
...
let proposal = parseInt(object_type.proposal, 10); // let balance_type  = parseInt(object_type.balance, 10);
...
let block_summary_prefix = "2." + parseInt(impl_object_type.block_summary, 10) + "."; // let vesting_balance_prefix = "1." + vesting_balance_type + ".";
...
class ChainStore {
  ...
  */
  ...
  init() {
    ...
    var _init = (resolve, reject) => {
      ...
      let db_api = _cybexjsWs.Apis.instance().db_api();
        ...
        for (let i = 0; i < optional_objects.length; i++) {
          ...
          if (optional_object) {
            ...
            */
            ...
            let delta = (now - head_time) / 1000; // let start = Date.parse("Sep 1, 2015");
            // let progress_delta = head_time - start;
            // this.progress = progress_delta / (now-start);

            if (delta < 60) {
              _cybexjsWs.Apis.instance().db_api().exec("set_subscribe_callback", [this.onUpdate.bind(this), true]).then(() => {
  ...
  */
  ...
  getAsset(id_or_symbol) {
    ...
    _cybexjsWs.Apis.instance().db_api().exec("lookup_asset_symbols", [[id_or_symbol]]).then(asset_objects => {
  ...
  */
  ...
  getAccountRefsOfKey(key) {
    if (this.get_account_refs_of_keys_calls.has(key)) return this.account_ids_by_key.get(key);else {
      ...
      _cybexjsWs.Apis.instance().db_api().exec("get_key_references", [[key]]).then(vec_account_id => {
        let refs = _immutable.default.Set();
  ...
  */
  ...
  getAccountRefsOfAccount(account_id) {
    if (this.get_account_refs_of_accounts_calls.has(account_id)) return this.account_ids_by_account.get(account_id);else {
      ...
      _cybexjsWs.Apis.instance().db_api().exec("get_account_references", [account_id]).then(vec_account_id => {
        let refs = _immutable.default.Set();
  ...
  getBalanceObjects(address) {
    ...
    if (current === undefined) {
      ...
      */
      ...
      _cybexjsWs.Apis.instance().db_api().exec("get_balance_objects", [[address]]).then(balance_objects => {
        let set = new Set();
        ...
        this.balance_objects_by_address.set(address, _immutable.default.Set(set));
        this.notifySubscribers();
      }, () => {
  ...
  */
  ...
  fetchObject(id, force = false, autosubscribe = true) {
    if (typeof id !== "string") {
      ...
      for (let i = 0; i < id.length; ++i) result.push(this.fetchObject(id[i], force, autosubscribe));
    ...
    if (result === undefined) {
      ...
      if (!_cybexjsWs.Apis.instance().db_api()) return null;

      _cybexjsWs.Apis.instance().db_api().exec("get_objects", [[id]]).then(optional_objects => {
        //if(DEBUG) console.log("... optional_objects",optional_objects ? optional_objects[0].id : null)
        for (let i = 0; i < optional_objects.length; i++) {
          let optional_object = optional_objects[i];
  ...
  */
  ...
  fetchWitnessByAccount(account_id) {
    return new Promise((resolve, reject) => {
      _cybexjsWs.Apis.instance().db_api().exec("get_witness_by_account", [account_id]).then(optional_witness_object => {
  ...
  */
  ...
  fetchCommitteeMemberByAccount(account_id) {
    return new Promise((resolve, reject) => {
      _cybexjsWs.Apis.instance().db_api().exec("get_committee_member_by_account", [account_id]).then(optional_committee_object => {
  ...
  */
  ...
  fetchFullAccount(name_or_id, autosubscribe = true) {
    ...
    if (_ChainValidation.default.is_object_id(name_or_id) && !subChanged) {
      ...
      if (!_ChainValidation.default.is_account_name(name_or_id, true)) throw Error("argument is not an account name: " + name_or_id);
      let account_id = this.accounts_by_name.get(name_or_id);
    ...
    if (subChanged || !this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
      this.fetching_get_full_accounts.set(name_or_id, Date.now()); // console.log( "FETCHING FULL ACCOUNT: ", name_or_id, autosubscribe );

      _cybexjsWs.Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], autosubscribe]).then(results => {
        if (results.length === 0) {
          if (_ChainValidation.default.is_object_id(name_or_id)) {
        ...
        if (sub_to_objects.length) _cybexjsWs.Apis.instance().db_api().exec("get_objects", [sub_to_objects]);
        ...
        console.log("Error: ", error);
        if (_ChainValidation.default.is_object_id(name_or_id)) this.objects_by_id.delete(name_or_id);else this.accounts_by_name.delete(name_or_id);
      });
    }
  ...
  */
  ...
  fetchRecentHistory(account, limit = 100) {
    ...
    pending_request.promise = new Promise((resolve, reject) => {
      _cybexjsWs.Apis.instance().history_api().exec("get_account_history", [account_id, most_recent, limit, start]).then(operations => {
        ...
        let updated_history = _immutable.default.fromJS(operations);
        ...
        this.objects_by_id.set(account_id, updated_account); //if( current_history != updated_history )
  ...
  */
    ...
    */
    ...
    if (object.id.substring(0, balance_prefix.length) == balance_prefix) {
      ...
      if (!dynamic) {
        ...
        if (!dad) dad = _immutable.default.Map();
      ...
      if (account) {
        if (!account.has("call_orders")) account = account.set("call_orders", new _immutable.default.Set());
        let call_orders = account.get("call_orders");
        ...
        if (!call_orders.has(object.id)) {
          ...
          _cybexjsWs.Apis.instance().db_api().exec("get_objects", [[object.id]]); // Force subscription to the object in the witness node by calling get_objects
      ...
      if (account) {
        if (!account.has("orders")) account = account.set("orders", new _immutable.default.Set());
        let limit_orders = account.get("orders");
  ...
  getObjectsByVoteIds(vote_ids) {
    ...
    if (missing.length) {
      ...
      _cybexjsWs.Apis.instance().db_api().exec("lookup_vote_ids", [missing]).then(vote_obj_array => {
  ...
  addProposalData(approvals, objectId) {
    approvals.forEach(id => {
      ...
      if (impactedAccount) {
        let proposals = impactedAccount.get("proposals", _immutable.default.Set());
...
function FetchChain(methodName, objectIds, timeout = 1900, subMap = {}) {
  ...
  return chain_store.FetchChainObjects(method, _immutable.default.List(objectIds), timeout, subMap).then(res => arrayIn ? res : res.get(0));
}
