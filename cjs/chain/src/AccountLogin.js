...
class AccountLogin {
  ...
  generateKeys(accountName, password, roles, prefix) {
    ...
    (roles || this.get("roles")).forEach(role => {
      ...
      let pkey = _keyCachePriv[seed] ? _keyCachePriv[seed] : _PrivateKey.default.fromSeed(_KeyUtils.default.normalize_brainKey(seed));
  ...
  checkKeys({
    ...
    for (let role in auths) {
      let {
        privKeys,
        pubKeys
      } = this.generateKeys(accountName, password, [role]);
