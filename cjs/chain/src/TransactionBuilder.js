...
class TransactionBuilder {
  ...
  process_transaction(cwallet, signer_pubkeys = null, broadcast = false) {
    ...
    if (_cybexjsWs.Apis.instance().chain_id !== wallet_object.get("chain_id")) return Promise.reject("Mismatched chain_id; expecting " + wallet_object.get("chain_id") + ", but got " + _cybexjsWs.Apis.instance().chain_id);
  ...
  finalize() {
    return new Promise((resolve, reject) => {
      ...
      resolve(_cybexjsWs.Apis.instance().db_api().exec("get_objects", [["2.1.0"]]).then(r => {
        head_block_time_string = r[0].time;
        if (this.expiration === 0) this.expiration = base_expiration_sec() + _cybexjsWs.ChainConfig.expire_in_secs;
        this.ref_block_num = r[0].head_block_number & 0xffff;
        this.ref_block_prefix = new Buffer(r[0].head_block_id, "hex").readUInt32LE(4); //DEBUG console.log("ref_block",@ref_block_num,@ref_block_prefix,r)
        ...
        this.tr_buffer = _serializer.ops.transaction.toBuffer(this);
      }));
    });
  }
  ...
  get_type_operation(name, operation) {
    ...
    if (name === "proposal_create") {
      ...
      operation.expiration_time || (operation.expiration_time = base_expiration_sec() + _cybexjsWs.ChainConfig.expire_in_secs_proposal);

      if (requiresReview) {
        operation.review_period_seconds = extraReview + Math.max(committee_min_review, 24 * 60 * 60 || _cybexjsWs.ChainConfig.review_in_secs_committee);
        /*
  ...
  update_head_block() {
    return Promise.all([_cybexjsWs.Apis.instance().db_api().exec("get_objects", [["2.0.0"]]), _cybexjsWs.Apis.instance().db_api().exec("get_objects", [["2.1.0"]])]).then(function (res) {
      let [g, r] = res;
      head_block_time_string = r[0].time;
  ...
  set_required_fees(asset_id) {
    ...
    let promises = [_cybexjsWs.Apis.instance().db_api().exec("get_required_fees", [operations, asset_id])];

    if (asset_id !== "1.3.0") {
      // This handles the fallback to paying fees in CYB if the fee pool is empty.
      promises.push(_cybexjsWs.Apis.instance().db_api().exec("get_required_fees", [operations, "1.3.0"]));
      promises.push(_cybexjsWs.Apis.instance().db_api().exec("get_objects", [[asset_id]]));
    }
    ...
    return Promise.all(promises).then(results => {
      ...
      let dynamicPromise = asset_id !== "1.3.0" && asset ? _cybexjsWs.Apis.instance().db_api().exec("get_objects", [[asset.dynamic_asset_data_id]]) : new Promise(resolve => resolve());
  ...
  get_potential_signatures() {
    ...
    return Promise.all([_cybexjsWs.Apis.instance().db_api().exec("get_potential_signatures", [tr_object]), _cybexjsWs.Apis.instance().db_api().exec("get_potential_address_signatures", [tr_object])]).then(function (results) {
      return {
        pubkeys: results[0],
        addys: results[1]
      };
  ...
  get_required_signatures(available_keys) {
    ...
    return _cybexjsWs.Apis.instance().db_api().exec("get_required_signatures", [tr_object, available_keys]).then(function (required_public_keys) {
  ...
  sign(chain_id = _cybexjsWs.Apis.instance().chain_id) {
    if (!this.tr_buffer) {
      throw new Error("not finalized");
    }
...
function _broadcast(was_broadcast_callback) {
  return new Promise((resolve, reject) => {
    ...
    var tr_object = _serializer.ops.signed_transaction.toObject(this); // console.log('... broadcast_transaction_with_callback !!!')


    _cybexjsWs.Apis.instance().network_api().exec("broadcast_transaction_with_callback", [function (res) {
      return resolve(res);
      ...
      reject(new Error(message + "\n" + "bitshares-crypto " + " digest " + _ecc.hash.sha256(this.tr_buffer).toString("hex") + " transaction " + this.tr_buffer.toString("hex") + " " + JSON.stringify(tr_object)));
      return;
    });
